...
import signupFrom from "./components/signupFrom.js";
import { Modal } from "flowbite";

import "flowbite";
import "./components/mainContainer.js";
import "../css/signup-with-button-style.css";

window.Alpine = Alpine;
Alpine.plugin(mask);
let signupModal;
document.addEventListener("DOMContentLoaded", () => {
  const mainContainer = document.getElementById("wlws_main_container");
  const notifContainer = document.getElementById(
    "wlws_main_container_notification",
  );
  const signupButton = document.getElementById("signup_button");
  const signupForm = signupFrom();
  mainContainer.append(signupForm);
  signupModal = new Modal(signupForm, {
    backdropClasses:
      "bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-[99999]",
    placement: "center",
    backdrop: "static",
    onShow: () => {
      mainContainer.classList.add("z-[999999]");
      notifContainer.classList.add("z-[9999999]", "md:z-[999999]");
    },
    onHide: () => {
      minContainer.classList.remove("z-[999999]");
      notifContainer.classList.remove("z-[9999999]", "md:z-[999999]");
    },
  });
