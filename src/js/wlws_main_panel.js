$(function ($) {
  ...
  let ids = [];
  $.getJSON(`${wlws.plugin_url}/src/json/panel.json`).done(
    async function (data) {
  ...
  async function create_container(id, container) {
    ...
    create_container.addClass(
      "w-full h-full bg-sky-50 rounded-2xl hidden wlws_panel_container overflow-y-auto ",
    );
  ...
  wlws_settings.on("submit", async function (e) {
    ...
    wlws_submit_button
      .prop("disabled", true)
      .toggleClass("cursor-pointer cursor-not-allowed active")
      .find("svg")
      .toggleClass("hidden");
